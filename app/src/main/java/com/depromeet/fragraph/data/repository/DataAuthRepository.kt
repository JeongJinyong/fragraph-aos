...
class DataAuthRepository @Inject constructor(
    ...
    override fun loginWithKakao(kakaoToken: String): Flow<UserCreated> {
        return flow {
            fragraphApi.loginWithSocial("kakao", SocialLoginRequest(kakaoToken))
                .getBodyOrThrow()?.let {
                context
                    .authSharedPreferences()
                    .edit()
                    .putString(KEY_AUTH_TOKEN, it.data.jwt)
                    .commit()

                emit(it.data.userCreated)
            }
        }.flowOn(Dispatchers.IO) // Use the IO thread for this Flow
    }

    override fun loginWithGoogle(googleToken: String): Flow<UserCreated> {
        return flow {
            fragraphApi.loginWithSocial("google", SocialLoginRequest(googleToken))
                .getBodyOrThrow()?.let {
                context
                    .authSharedPreferences()
                    .edit()
